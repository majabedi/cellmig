...
FixStyle(cell/migrate,FixCellMigration)

#else

#ifndef LMP_FIX_CELL_MIG_H
#define LMP_FIX_CELL_MIG_H

#include "fix.h"

namespace LAMMPS_NS {

class FixCellMigration: public Fix {
public:
	FixCellMigration(class LAMMPS *, int, char **);
	~FixCellMigration();
	int setmask();
	void init();
...
private:
	...
	int inewtype, jnewtype, iatomOriginaltype;
	...
	int nucleusatomtype;
	int insidebondprotured;
	int insidebondnormal;
