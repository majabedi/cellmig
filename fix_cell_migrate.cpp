...
FixCellMigration::FixCellMigration(LAMMPS *lmp, int narg, char **arg) :
		Fix(lmp, narg, arg), bondcount(NULL), partner(NULL), finalpartner(NULL), distsq(
				NULL), probability(NULL), created(NULL), copy(NULL), random(
				NULL), list(NULL) {
	if (narg < 8)
	...
	if (strcmp(arg[9], "x") == 0) {
		...
		error->all(FLERR, "Invalid direction of polarization");
	...
	if (iatomtype < 1 || iatomtype > atom->ntypes || jatomtype < 1
			|| jatomtype > atom->ntypes || nucleusatomtype < 1
			|| nucleusatomtype > atom->ntypes)
		error->all(FLERR, "Invalid atom type in fix bond/create command");
	if (cutoff < 0.0)
		error->all(FLERR, "Illegal fix bond/create command");
	if (btype < 1 || btype > atom->nbondtypes || insidebondnormal < 1
			|| insidebondnormal > atom->nbondtypes || insidebondprotured < 1
			|| insidebondprotured > atom->nbondtypes)
...
void FixCellMigration::post_integrate() {
		...
		for (jj = 0; jj < jnum; jj++) {
			...
			if (itype == iatomtype && jtype == jatomtype) {
				if ((imaxbond == 0 || bondcount[i] < imaxbond)
					...
					possible = 1;
				int btypeLinks = 0;
				for (int ii = 0; ii < num_bond[i]; ii++)
					if (bond_type[i][ii] == btype)
						btypeLinks++;
				if (btypeLinks > 0)
					possible = 0;
				...
				for (int ii = 0; ii < num_bond[j]; ii++)
					if (bond_type[j][ii] == btype)
						btypeLinks++;
				if (btypeLinks > 0)
					possible = 0;
	...
	for (i = 0; i < nlocal; i++) {
		...
		if (!newton_bond || tag[i] < tag[j]) {
			...
			num_bond[i]++;
			printf(
					"Created a bond of type %d between %d-%d and the current number of bonds for %d is %d and for %d is %d.\n",
					btype, tag[i], tag[j], tag[i], num_bond[i], tag[j],
					num_bond[j]);
				...
				for (int ij = 0; ij < num_bond[i]; ij++) {
					if (bond_type[i][ij] == insidebondprotured) {
						...
						if (newton_bond) {
							...
							if (tag[i] < tag[nucleusLocalIndex])
								bond_type[i][ij] = insidebondnormal;
							else
								for (int localI = 0;
										localI < num_bond[nucleusLocalIndex];
										localI++)
									if (bond_atom[nucleusLocalIndex][localI]
											== tag[i])
										bond_type[nucleusLocalIndex][localI] =
												insidebondnormal;
						...
						printf(
								"Changed atom type of %d to %d and bondtype of %d to %d in upper loop\n",
								type[i], iatomOriginaltype, insidebondprotured,
								insidebondnormal);
				...
				for (int ii = 0; ii < nlocal; ii++)
					if (type[ii] == nucleusatomtype)
						for (int ij = 0; ij < num_bond[ii]; ij++) {
							if (bond_type[ii][ij] == insidebondprotured
									&& bond_atom[ii][ij] == tag[i]) {
								if (newton_bond) {
									printf(
											"Trying to deprotrude the link %d,%d\n",
											tag[i], tag[ii]);
									bond_type[ii][ij] = insidebondnormal;
								} else {
									bond_type[i][ij] = insidebondnormal;
									for (int localI = 0; localI < num_bond[ii];
											localI++)
										if (bond_atom[ii][localI] == tag[i])
											bond_type[ii][localI] =
													insidebondnormal;
								}
								printf(
										"Changed atom type of %d to %d and bondtype of %d to %d in lower loop\n",
										type[i], iatomOriginaltype,
										insidebondprotured, insidebondnormal);
...
void FixCellMigration::print_copy(const char *str, tagint m, int n1, int n2,
int n3, int *v) {
