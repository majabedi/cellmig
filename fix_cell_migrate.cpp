...
FixCellMigration::FixCellMigration(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  ...
  probability(NULL), created(NULL), copy(NULL), random(NULL), list(NULL)
{
  ...
  nucleusatomtype = force->inumeric(FLERR,arg[4]);
  //HERE to add the new extended type
  iatomOriginaltype = force->inumeric(FLERR,arg[5]);
  iatomtype = force->inumeric(FLERR,arg[6]);
  jatomtype = force->inumeric(FLERR,arg[7]);
  double cutoff = force->numeric(FLERR,arg[8]);

  if (strcmp(arg[9],"x")==0){
	migrationdirection = 0;
  }  else if (strcmp(arg[9],"y")==0){
		migrationdirection = 1;
	} else if (strcmp(arg[9],"z")==0){
		migrationdirection = 2;
	} else error->all(FLERR,"Invalid direction of polarization");

  fraction = force->numeric(FLERR,arg[10]);
  btype = force->inumeric(FLERR,arg[11]);
  insidebondnormal = force->inumeric(FLERR,arg[12]);
  insidebondprotured = force->inumeric(FLERR,arg[13]);

  if (iatomtype < 1 || iatomtype > atom->ntypes ||
      jatomtype < 1 || jatomtype > atom->ntypes ||
      nucleusatomtype < 1 || nucleusatomtype > atom->ntypes)
  ...
  if (btype < 1 || btype > atom->nbondtypes ||
      insidebondnormal < 1 || insidebondnormal > atom->nbondtypes ||
      insidebondprotured < 1 || insidebondprotured > atom->nbondtypes )
...
FixCellMigration::~FixCellMigration()
{
...
void FixCellMigration::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void FixCellMigration::setup(int vflag)
{
...
void FixCellMigration::post_integrate()
{
  ...
  for (i = 0; i < nlocal; i++) {
      //printf(" %d is considered \n", tag[i]);
      
      if (type[i] == nucleusatomtype) {
          //printf(" %d is nucleus\n", tag[i]);
          double mostFront = -BIG;
          int mostFrontIndex = -1;
          // std::cin >> mostFrontIndex;
          bool isProtured = false;
          int localIndex;
          for (j = 0; j < num_bond[i]; j++) {
              //printf("BEFORE: %d has bond type %d,%d\n", tag[bond_atom[i][j]],bond_type[i][j],bond_type[j][i]);         
              if (bond_type[i][j] == insidebondprotured) {
                  isProtured = true;
                  continue;
              }
              localIndex = atom -> map(bond_atom[i][j]);
              if ( x[localIndex][migrationdirection] > mostFront ) {
                  
                  mostFront = x[localIndex][migrationdirection] ;
                  mostFrontIndex = j;
              }
          ...
          if (!isProtured) {
              printf("Wants to protrude element %d\n", bond_atom[i][mostFrontIndex] );
              type[atom -> map(bond_atom[i][mostFrontIndex])] = iatomtype;
              //type[atom -> map(bond_atom[i][j])] = iatomtype;
              if (newton_bond)
                  if(tag[i] < bond_atom[i][mostFrontIndex]) {
                      bond_type[i][mostFrontIndex] = insidebondprotured;
                      
                  }
                  else {
                     
                      bond_type[mostFrontIndex][i] = insidebondprotured;
...
///////////////////
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (itype == iatomtype && jtype == jatomtype) {
        if ((imaxbond == 0 || bondcount[i] < imaxbond) &&
          ...
          possible = 1;
        int btypeLinks = 0;
        for (int ii=0;ii<num_bond[i];ii++) if (bond_type[i][ii]==btype) btypeLinks++;
        if (btypeLinks>0) possible = 0;
        ...
        for (int ii=0;ii<num_bond[j];ii++) if (bond_type[j][ii]==btype) btypeLinks++;
        if (btypeLinks>0) possible = 0;

      }
      //else printf("Bond count for %d is %d so it is not possible\n",tag[i],bondcount[i]);
  ...
  for (i = 0; i < nlocal; i++) {
	...
	double meanX = 0;
	int nlocal = atom->nlocal;
	double localAvgVel[2]; // 4th element for particles count
	double globalAvgVel[2];
	double localMin[1];
	double localMax[1];
	memset(localAvgVel, 0, 2 * sizeof(double));
	memset(localMin, 0, sizeof(double));
	memset(localMax, 0, sizeof(double));
	
	/*for (int particleInd = 0; particleInd < nlocal; ++particleInd) {
		meanX += x[particleInd][0];
	if(particleInd == 0){
		localMin[0] = x[particleInd][0];
		localMax[0] = x[particleInd][0];
	} else {
		if(localMin[0] > x[particleInd][0]) localMin[0] = x[particleInd][0];
		if(localMax[0] < x[particleInd][0]) localMax[0] = x[particleInd][0];
	}
	}
	localAvgVel[0]=meanX;
	localAvgVel[1]=nlocal;



	double minX[1];
	memset(minX, 0, sizeof(double));
	double maxX[1];
	memset(maxX, 0, sizeof(double));
	
	MPI_Allreduce(localAvgVel, globalAvgVel, 2, MPI_DOUBLE, MPI_SUM, world);
	MPI_Allreduce(localMin, minX, 1, MPI_DOUBLE, MPI_MIN, world);
	MPI_Allreduce(localMax, maxX, 1, MPI_DOUBLE, MPI_MAX, world);

	if (comm->me == 0) {
	printf("the total x is \%f for total number of particles \%f\n",
	globalAvgVel[0], globalAvgVel[1]);
	}
	printf("Prob for %d,%d should be %f,%f",tag[i],tag[j],(fraction*x[i][0]-minX[0])/(maxX[0]-minX[0]),(fraction*x[j][0]-minX[0])/(maxX[0]-minX[0]));
	printf("min max is %f,%f",minX[0],maxX[0]);*/
    ...
    if (!newton_bond || tag[i] < tag[j]) {
      ...
      num_bond[i]++;
      printf("Created a bond of type %d between %d-%d and the current number of bonds for %d is %d and for %d is %d.\n",btype,tag[i],tag[j],tag[i],num_bond[i],tag[j],num_bond[j]);
    ...
    bondcount[i]++;
      //printf("Created a bond of type %d between %d-%d amd the current number of bonds for %d is %d and for %d is %d.\n",btype,tag[i],tag[j],tag[i],num_bond[i],tag[j],num_bond[j]);
      if (type[i] == iatomtype) {
          //If a bond is created make it the original type so that it does not absorb anymore
          printf("Trying to deprotrude\n");
          if (bondcount[i] > 0 ) {
              for (int ij = 0; ij < num_bond[i]; ij++){
                  if (bond_type[i][ij] == insidebondprotured) {
                      int nucleusLocalIndex = atom->map(bond_atom[i][ij]);
                      if(newton_bond){
                                    printf("Trying to deprotrude the link %d,%d\n",tag[i],tag[nucleusLocalIndex]);

                          if(tag[i]<tag[nucleusLocalIndex]) bond_type[i][ij] = insidebondnormal;
                          else for(int localI=0;localI<num_bond[nucleusLocalIndex];localI++) if(bond_atom[nucleusLocalIndex][localI] == tag[i]) bond_type[nucleusLocalIndex][localI] = insidebondnormal;
                          
                    }
                    else {
                          bond_type[i][ij] = insidebondnormal;
                          for(int localI=0;localI<num_bond[nucleusLocalIndex];localI++) if(bond_atom[nucleusLocalIndex][localI] == tag[i]) bond_type[nucleusLocalIndex][localI] = insidebondnormal;
                      }
                      printf("Changed atom type of %d to %d and bondtype of %d to %d in upper loop\n", type[i],iatomOriginaltype,insidebondprotured,insidebondnormal);
                      
                }
              //printf("here checked the bond of %d with %d\n", tag[i],bond_atom[i][ij]);
        }
        ///////////
        for (int ii = 0;ii<nlocal;ii++)
            if (type[ii] == nucleusatomtype )
        for (int ij = 0; ij < num_bond[ii]; ij++){
                  if (bond_type[ii][ij] == insidebondprotured && bond_atom[ii][ij]==tag[i]) {
                      if(newton_bond){
                                    printf("Trying to deprotrude the link %d,%d\n",tag[i],tag[ii]);
                                    bond_type[ii][ij] = insidebondnormal;

                         // if(tag[i]<tag[ii]) bond_type[i][ij] = insidebondnormal;
                         // else for(int localI=0;localI<num_bond[ii];localI++) if(bond_atom[ii][localI] == tag[i]) bond_type[ii][localI] = insidebondnormal;
                          
                    }
                    else {
                          bond_type[i][ij] = insidebondnormal;
                          for(int localI=0;localI<num_bond[ii];localI++) if(bond_atom[ii][localI] == tag[i]) bond_type[ii][localI] = insidebondnormal;
                      }
                      printf("Changed atom type of %d to %d and bondtype of %d to %d in lower loop\n", type[i],iatomOriginaltype,insidebondprotured,insidebondnormal);
                      
                }
              //printf("here checked the bond of %d with %d\n", tag[ii],bond_atom[ii][ij]);
        }
        ///////////
              type[i] = iatomOriginaltype;
                  }
      } else {
      if (bondcount[i] == jmaxbond) {
	type[i] = jnewtype;
        printf("Max %d is reached for atom j=%d of type %d\n",jmaxbond,tag[i],type[i]);
      }
    }
...
void FixCellMigration::check_ghosts()
{
  int i,j,n;
  tagint *slist;
...
void FixCellMigration::update_topology()
{
...
void FixCellMigration::rebuild_special_one(int m)
{
...
void FixCellMigration::create_angles(int m)
{
...
void FixCellMigration::create_dihedrals(int m)
{
...
void FixCellMigration::create_impropers(int m)
{
...
int FixCellMigration::dedup(int nstart, int nstop, tagint *copy)
{
  int i;
...
void FixCellMigration::post_integrate_respa(int ilevel, int iloop)
{
...
int FixCellMigration::pack_forward_comm(int n, int *list, double *buf,
...
void FixCellMigration::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,m,ns,last;
...
int FixCellMigration::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixCellMigration::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixCellMigration::grow_arrays(int nmax)
{
...
void FixCellMigration::copy_arrays(int i, int j, int delflag)
{
...
int FixCellMigration::pack_exchange(int i, double *buf)
{
  buf[0] = bondcount[i];
  return 1;
}
...
int FixCellMigration::unpack_exchange(int nlocal, double *buf)
{
...
double FixCellMigration::compute_vector(int n)
{
...
double FixCellMigration::memory_usage()
{
  int nmax = atom->nmax;
...
void FixCellMigration::print_bb()
{
...
void FixCellMigration::print_copy(const char *str, tagint m,
